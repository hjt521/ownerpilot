...
| 5 | RIE-001 | shipped (dark — `BTRM_STAGE_RIE_ENABLED` unset everywhere; no caller wired yet) |
| 6 | OCM-001 / CS-001 | shipped (dark — `BTRM_STAGE_OCM_ENABLED` / `BTRM_STAGE_CS_ENABLED` unset everywhere; no caller wired yet) |
| 7 | POL-001 | shipped (dark — `BTRM_STAGE_POL_ENABLED` unset everywhere; no caller wired yet) |

**All seven build stages are now shipped, dark.** The full BTRM-001 component pipeline (§1) exists in code, flag-gated off end to end. No stage is wired to any consumer surface (Chat/RiskPath/Notice) — that is a separate, explicitly-approved integration step per §10.
...
- **Stage 5 implementation note (2026-07-26, `lib/btrm/rie/`):** identifying what a candidate resolution option even is, and why it is being considered, remains the caller's job — the same posture ENR-001/BAE-001/ICOA-001's own hints already establish. RIE-001 accepts an explicit `ResolutionOptionHint` (a closed `type` plus caller-supplied purpose/expected-benefit/documentation text, plus references to the specific `InterestConstraint` and `RelianceAssessment` ids it is built on) and grades it strictly deterministically: `reversibility` and `materialConsequence` are fixed by the option's `type` alone (`lib/btrm/rie/catalog.ts`, a closed lookup table — never inferred from hint content), while `requiredConditions`/`missingInformation`/`materialRisks`/`relianceAssumptions` are derived only from cited ids that actually resolve against the supplied evidence scope — an id that does not resolve contributes nothing, the same "no fabricated support" rule ICOA-001 already applies to its own hint references. `materialConsequence` classification maps BTRM-001 §6's category list (formal notice, termination, filing prep, settlement terms, material financial concession) onto the ten option types; two types (`payment_plan`, `repair_and_rent_coordination`) require a judgment call because they carry financial/rent terms without being an unambiguous single §6 category — both are classified `materialConsequence: true`, erring toward the extra human-review gate rather than risk a consequential option skipping review (see `lib/btrm/rie/catalog.ts` header for the full rationale; this is a Stage 5 implementation decision, not a constitutional ruling). Every option's explainability envelope (spec §5) is asserted complete before the option is ever returned — a hint that cites no interest or reliance reference at all has nothing to ground its conclusion in and is correctly rejected rather than shipped as a black-box result. Shipped dark: `BTRM_STAGE_RIE_ENABLED` is unset everywhere and no caller invokes `options()`/`evaluateOption()` yet.
...
- **Stage 6 implementation note (2026-07-26, `lib/btrm/ocm/`):** the caller designates which `ResolutionOption` is the negotiated path and which represent the likely alternative(s) — OCM-001 does not decide this itself, mirroring every other BTRM-001 hint. Internally, an aggregate numeric signal is computed per option from its resolved `RelianceAssessment` levels, their linked `ConfidenceAssessment` bands, and its own counted `materialRisks`/`missingInformation` — this is exactly the "permitted internal use only" telemetry RQS (§3.7.1) authorizes for calibration purposes; **the number itself is never returned, never exposed, and never itself ranks or approves anything.** Only a fixed-threshold-derived qualitative `SupportBand` and a plain-language rationale citing the actual structural facts (reliance levels, confidence bands, risk/gap counts) are returned. A primary option with zero resolvable reliance signal is `insufficient_evidence` regardless of the alternative — the same "a hard failure is not averaged away" rule applied throughout BTRM-001. `OutcomeComparison` gained one new field this stage (`envelope`), additive only. Shipped dark: `BTRM_STAGE_OCM_ENABLED` is unset everywhere and no caller invokes `compare()` yet.
...
- **Stage 6 implementation note (2026-07-26, `lib/btrm/cs/`):** CS-001 does not draft message prose — a caller supplies the structured content (`factsStatement`, optional `allegationsStatement`, `requestedAction`, `offeredOptions`), the same posture every other BTRM-001 hint already establishes. Facts and allegations are held in permanently **separate** fields on the output type (`CommunicationRecommendation`, newly defined here since spec §4's data model did not enumerate it) — never concatenated into one blob, satisfying "separate facts from allegations" structurally rather than by convention. Every free-text field is scanned by `assertNoCharacterLabel` before return. Reading-level, language-preference, and de-escalation adaptations are deterministic, templated notes (`lib/btrm/cs/adaptations.ts`) driven by a caller-supplied `AudienceContext` — CS-001 never infers audience characteristics itself, and never performs the translation it flags as needed. `humanReviewRequired` is inherited unchanged from the referenced `ResolutionOption`; CS-001 cannot lower a gate RIE-001 already set. `evidenceCited`/`behaviorsIdentified` on this component's own envelope are reused from the referenced option's already-validated envelope rather than re-derived, since CS-001 introduces no new evidentiary basis of its own. Shipped dark: `BTRM_STAGE_CS_ENABLED` is unset everywhere and no caller invokes `strategy()` yet.
...
- **Stage 7 implementation note (2026-07-26, `lib/btrm/pol/`):** `OutcomeRecord` gained one new field this stage, `recencyWeight` (additive, `[0, 1]`) — a decay weight computed only for use if/when this record re-enters BAE-001's observation stream on a **future** assessment; it is never itself a stored per-person score (spec §3.9's absolute rule), and is always recomputed relative to a caller-supplied `asOf` reference time, never `Date.now()`, mirroring every other BTRM-001 component's determinism convention. Both recency AND relevance gate the weight, not recency alone: a caller-supplied `relevantToCurrentClaim` hint (POL-001 does not infer relevance itself) zeroes the weight outright regardless of how fresh the outcome is, directly implementing "recent, relevant behavior outweighs old or unrelated behavior" rather than recency in isolation. `contextNotes` is preserved verbatim — never dropped, summarized, or rewritten — satisfying the emergency-context-preservation rule structurally; it is scanned only for the §11 no-character-label safeguard. This stage does **not** wire the feedback loop itself (spec §1's "feeds ENR/BAE on next assessment" remains a future, separately-scoped integration) — `record()` only produces the `OutcomeRecord`. Shipped dark: `BTRM_STAGE_POL_ENABLED` is unset everywhere and no caller invokes `record()` yet.

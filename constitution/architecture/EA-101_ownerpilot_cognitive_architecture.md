...
created: 2026-07-26
updated: 2026-07-26
depends_on: [EA-100, EA-012, BTRM-001, EA-102, IMR-001, ADR-015, ADR-016, ADR-017, ADR-018]
required_by: []
implements: [EA-100]
...
supersedes: []
superseded_by: []
related_artifacts: [RP-005, RP-006, RP-007, RP-008, RPT-012, RPT-013, RPT-015, RPT-016, RPT-017, CKG-001, ICOA-001, RIE-001, OCM-001, CS-001, POL-001, ENR-001, BAE-001, TM-001, CM-001, DOC-003]
registry_tags: [cognitive-architecture, ea-101, opos, reasoning-pipeline, architecture-draft]
program_phase: enterprise-delivery
repository_path: constitution/architecture/EA-101_ownerpilot_cognitive_architecture.md
checksum_scope: file
---

# EA-101 — OwnerPilot Cognitive Architecture: How OwnerPilot Thinks (Architecture Draft)

> **Lifecycle: Architecture Draft — not yet constitutional, not yet authorized to build.** Authored per RP-008's Founder directive and RPT-017's reconciliation of RP-006/RP-008, following the same lifecycle BTRM-001 and EA-102 already used: Architecture Draft → Founder Review → self-critique/review-board challenge (§13–14, included below) → ADR → Ratification. This document adopts RPT-017's recommended defaults throughout and states, in §16, exactly which of those defaults still require Founder confirmation rather than presenting them as already decided.
...
Per RPT-017 §3's recommendation, OPOS is specified here as **EA-101's own internal specification, not a separate CRID**, on the reasoning that it has no inputs, outputs, versioning need, or consumer distinct from what EA-101 must already state to describe how OwnerPilot reasons — the same "subordinate concept, not a fourth layer" discipline EA-102 §2.0 applied to its own Normalized Learning Contract. **This is flagged for Founder confirmation in §16 — it is EA-101's working assumption, not a settled fact.**

OPOS is the fixed operational sequence every recommendation passes through:
...
**OPOS's own hard rule:** no engine in §3 may skip a step, invent an upstream step's output, or bypass `human_review_required` once any upstream step has set it. This is not new — it restates BTRM-001 §6/§11 and EA-102 §8 — but it is OPOS's rule to enforce as the kernel, not each engine's to individually remember.
...
| **Financial Intelligence Engine** | **Genuinely new** (RPT-016 §4). No existing owner. Scoped but not designed here — see §16. Any probability/expected-value output it produces (e.g. "collection probability") is built under the existing ADR-015/BTRM-001 §3.7.1 qualitative-first disposition, unmodified, per RPT-016 §2. |
...
Defined by reference, not restated: **`constitution/implementation-specs/recommendation_object_spec_v0.1.md`** — ten binding semantic invariants reconciling RP-006's and RP-008's divergent field lists, held as a subordinate implementation spec rather than fixed constitutional prose, per the same discipline ADR-017 required of EA-102 §2.0. Whether Recommendation Object becomes its own CRID or remains defined entirely inside this document is open — see §16.

## 5. Decision Graph

Defined by reference: **`constitution/implementation-specs/decision_graph_spec_v0.1.md`** — a read-only query/traversal layer over ICOA-001 → RIE-001 → OCM-001 → EA-102's already-real relationships, no new write path. Explicitly distinct from **CKG-001** (the P5.5 Constitutional Knowledge Graph): CKG-001 connects constitutional/governance objects platform-wide; Decision Graph traces one matter's own reasoning. The two never share a node/edge vocabulary.
...
## 11. ECAP refactor — scheduled, not additive-only

Per RPT-016 §10, the four already-delivered Wave-1 capabilities (ECAP-001 AI Assistant, ECAP-002 Document Generation, ECAP-003 Serve & Track, ECAP-010 Evidence Management) were built and released before this pipeline existed. EA-101 schedules — but does not perform — a future Wave-1-refactor phase to consume OPOS/OPIL rather than independent reasoning, sequenced after the reasoning-engine pipeline itself exists (§3), per RPT-016 §11's own dependency sketch.
...
|---|---|---|---|
| EA-101 | OwnerPilot Cognitive Architecture | Top-level statement of how OwnerPilot reasons | New (this draft) |
| — | OPOS | The reasoning kernel/sequence | New concept, held as EA-101's internal spec (pending §16) |
| — | Financial Intelligence Engine | Cost/value/portfolio reasoning | Genuinely new — design deferred, own future Architecture Draft |
| — | Recommendation Object | Canonical recommendation shape | Spec'd in `implementation-specs/recommendation_object_spec_v0.1.md` — CRID status pending §16 |
| — | Decision Graph | Per-matter reasoning trace | Spec'd in `implementation-specs/decision_graph_spec_v0.1.md` — CRID status pending §16 |

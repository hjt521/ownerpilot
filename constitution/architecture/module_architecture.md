...
Legend: **[SD]** = a SECURITY DEFINER function participates. **[trig]** = has an `updated_at` trigger. (As of `const_0001` / Constitution v1.1, every table with an `updated_at` column carries a maintaining trigger — the former `[u-notrig]` coverage gap is closed.)
...
- **Tables (6):** `scenario_generation_requests` [trig], `scenario_generation_runs`, `generated_scenarios`, `scenario_templates` [trig], `scenario_variables`, `scenario_quality_reviews`.
...
- **Tables (9):** `simulation_actors` [trig], `actor_scenario_states`, `actor_response_distributions`, `behavioral_profiles` [trig], `behavioral_assessments`, `negotiation_cases` [trig], `negotiation_turns`, `negotiation_outcomes`, `negotiation_strategy_versions`.
...
- **Tables (9):** `decision_intelligence_requests` [trig], `decision_recommendations` (`recommendation_status`, `approval_status`, `confidence`, `confidence_band`, `approved_by`, `approved_at`), `decision_option_assessments`, `decision_sensitivity_findings`, `decision_explanation_factors`, `decision_explainability_audit`, `intelligence_evaluation_runs`, `intelligence_evaluation_suites` [trig], `intelligence_model_registry` [trig].

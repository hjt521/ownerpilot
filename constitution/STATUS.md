...
- **ECAP-002 Document Generation — DELIVERED through PROC-100 (RPT-009):** IP guards verified GREEN and CI-wired — locked-prose integrity (SHA-256 manifest, drift-failing), broker-only attribution (no attorney/SBN), banned-terms — validated; released. **No code change** (constitutional stakes already enforced by CI). Deferred: produce-path monitoring (post-flip). **Next:** ECAP-003.
...
- **Artifacts (with metadata):** 57 · **ADRs:** 12
- **By lifecycle:** Concept 12 · Implemented 3 · Operational 22 · Proposed 7 · Ratified 13

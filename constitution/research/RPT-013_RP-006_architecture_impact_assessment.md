...
| RQS (Recommendation Quality Score) — Fact/Legal/Negotiation/Outcome/Communication/Business Confidence | **OCM-001** (BTRM-001 §3.7/§15) | **RESOLVED 2026-07-25 by RPT-014, ratified by ADR-015** (original finding preserved below). Same unresolved conflict already flagged in RPT-012 §2, resurfacing under a new name at the time this table was written. OCM-001 was ratified as strictly qualitative (support bands) specifically to prohibit fabricated numeric confidence/probability output; RQS as originally proposed here scored six dimensions numerically. **Disposition now:** RQS is retained only as a qualitative-first, multidimensional assurance framework (seven dimensions: factual grounding, legal analysis, objective alignment, alternative consideration, risk analysis, communication strategy, execution readiness), no composite score, codified at BTRM-001 §3.7.1. Numeric telemetry is internal-diagnostic only. See RPT-014 for the full reconciliation. |
...
Unchanged from RPT-012's ordering, with two additions: resolve the OPOS/OPIL/EA-012 naming collision before either name is used in any future draft, and resolve EDIC's data-sourcing question explicitly before any corpus work begins. **The RQS-vs-OCM-001 conflict is now resolved — see the closure note in §1's RQS row and RPT-014.** No ratification, no CRID assignment, no implementation authorized by this report beyond the RQS resolution ratified separately via ADR-015.

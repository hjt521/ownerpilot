 ...
 *
 * CROSS-WORKSTREAM MIRROR (parcel-health): the parcel-health probe
 * (lib/jurisdiction/parcelHealth/endpoints/zimas.ts) imports buildZimasParcelQueryUrl,
 * parseZimasFeatures, and classifyZimasParcel from this file, so this module is in the
 * parcel-health _core build closure. Editing it requires regenerating
 * supabase/functions/parcel-health/_core/ (verify-parcel-health-core-sync) in the same PR.
 */
...
/**
 * Build the ArcGIS spatial point-in-polygon query URL for a WGS84 point.
 *
 * Extracted (behavior-preserving) from defaultZimasFetcher so the parcel-health probe
 * mirrors production's exact query — byte-identical URL, no hand-rolled reconstruction.
 * The geometry is `lng,lat` (ArcGIS wire order); inSR=4326 lets the server reproject.
 */
export function buildZimasParcelQueryUrl(point: { lng: number; lat: number }): string {
...
/** The ArcGIS landbase query response subset the adapter + probe read. */
export interface ZimasArcgisResponse {
  error?: unknown;
  features?: Array<{ attributes?: Record<string, unknown> }>;
}

/**
 * Normalize an ArcGIS landbase response to ZimasParcelRecord[].
 *
 * Extracted (behavior-preserving) from defaultZimasFetcher so the parcel-health probe
 * parses features identically to production (single source of truth into classifyZimasParcel).
 * Does NOT inspect json.error — callers handle that first (defaultZimasFetcher throws;
 * the parcel-health probe routes an error body to response_shape before calling this).
 */
export function parseZimasFeatures(json: ZimasArcgisResponse): ZimasParcelRecord[] {
...
export const defaultZimasFetcher: ZimasFetcher = async (lat, lng, signal) => {
  const url = buildZimasParcelQueryUrl({ lng, lat });
  const resp = await fetch(url, { method: 'GET', signal });
  if (!resp.ok) throw new Error(`zimas HTTP ${resp.status}`);
  const json = (await resp.json()) as ZimasArcgisResponse;
  if (json.error) throw new Error('zimas API error');
  return parseZimasFeatures(json);
};

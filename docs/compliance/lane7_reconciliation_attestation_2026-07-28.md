...
- **This row is the §3.6.a smoke test, not a separate/real Preview finding.** Evidence chain: the curl request above was issued at 19:10:xx UTC 2026-07-28 (immediately following `CRON_SECRET` rotation confirmation in the same working session); the row's `createdTime` is `2026-07-28 19:10:58Z` — direct timestamp correlation, not inference. Note this producer has no caller-supplied free-text marker field (unlike `/api/automation/log`'s `summary`, which callers author) — `geocode-audit`'s `RunRecord` is computed entirely server-side from live Supabase check results, so a `smoke_test_<id>` sentinel is not an available or applicable evidentiary convention for this specific producer. Timestamp + exact payload-content correlation (the three check values match verbatim between the HTTP response and the Notion row) is the evidence chain used here instead.

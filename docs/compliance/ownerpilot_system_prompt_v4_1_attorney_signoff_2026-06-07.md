# OwnerPilot System Prompt — v4.1 (Canonical Source of Record)

**Artifact:** `ownerpilot_system_prompt_v4_1_attorney_signoff_2026-06-07.md`
**Status:** Canonical source-of-record for the deployed `SYSTEM_PROMPT` in `app/api/chat/route.ts` as of 2026-06-07.
**Reviewer:** Janna Taglyan, JD, SBN 269639
**Date:** 2026-06-07
**Supersedes (on three sections only):** `ownerpilot_system_prompt_v4_final.md` (2026-05-30 sign-off) — superseded on SELF-HELP, CITATION HONESTY exception sentence, and DOCUMENTS interim language. v4-final remains the canonical source for every other section.
**Cross-references:**
- `ownerpilot_route_ts_chat_prompt_attorney_ruling.md` (2026-06-02 Round 1)
- `ownerpilot_route_ts_chat_prompt_attorney_ruling_round_2.md` (2026-06-02 Round 2 — verbatim amendment text)
- `ownerpilot_route_ts_chat_prompt_attorney_signoff.md` (2026-06-02 sign-off, four conditions)
- `system_prompt_drift_diff_attorney_ruling_2026-06-06.md` (drift hold — superseded by correction)
- `system_prompt_drift_diff_attorney_correction_2026-06-07.md` (drift hold retracted; commit d20ccc7 attribution accurate)
- `documents_interim_sunset_gate_attorney_signoff_2026-06-07.md` (sunset gate; condition (b) not met; CI guard approved)
- `uploaded_attachments/db247de259e14801bb50cf7365446218/ownerpilot_deployed_system_prompt_extracted_2026-06-06_1.md` (verbatim extraction this file mirrors)

---

## 1. Headline

**v4.1 is v4-final plus three attorney-approved 2026-06-02 amendments — nothing more, nothing less.** The deployed prompt body reproduced below is byte-identical to the extraction taken from production on 2026-06-06 (14,989 chars / 15,086 UTF-8 bytes between the BEGIN/END markers). Commit d20ccc7 (Jun 2 2026 18:48, `jack@ownerpilot.ai`, "port attorney-approved v4 prompt + add SELF-HELP section, DOCUMENTS interim language, and standing UI disclosure (attorney signed off 2026-06-02)") accurately attributes the changes to the 2026-06-02 Round 2 ruling chain.

**Route comment correction owed:** The in-route comment at `app/api/chat/route.ts` lines 9–10 currently reads:
...
This is now inaccurate on its face — the deployed body is v4.1, not v4-final. **[SHOULD FIX]** Update the comment to point at this file:
...
This is a comment-only change, no behavior impact, and it doesn't need a separate sign-off — fold it into the next route.ts commit.

---

## 2. Amendment register (v4-final → v4.1)

Three substantive amendments, all approved 2026-06-02 in the Round 2 ruling chain, all reproduced verbatim in the body below.

### Amendment A — SELF-HELP section (new)

- **Location in body:** "### SELF-HELP — never validate, always correct" section (one paragraph + bulleted prohibited-acts list + closing paragraph).
- **Source ruling:** `ownerpilot_route_ts_chat_prompt_attorney_ruling_round_2.md` (2026-06-02), SELF-HELP block. Sign-off in `ownerpilot_route_ts_chat_prompt_attorney_signoff.md` (2026-06-02), condition 1.
- **Status:** Permanent. No sunset.
- **Statutes cited inline (intentional exception to CITATION HONESTY — see Amendment B):** Civ. Code § 789.3 (lockout, utility shutoff); Civ. Code §§ 1980–1991 (lawful disposition of left-behind personal property, post-tenancy); Civ. Code § 1954 (entry); Civ. Code § 1940.3 (immigration-status inquiry prohibition); Civ. Code § 1942.5 (retaliation). All five are accurate as of 2026-06-02 verification.
- **[CONSIDER] carry-forward:** The bullet on "removing the tenant's belongings" describes §§ 1980–1991 as the lawful process "after a tenancy has lawfully ended." That's correct as a topline summary, but the next planned revision should consider tightening "lawfully ended" to make clear it covers both surrender and post-judgment recovery — the current wording reads slightly ambiguously on whether voluntary abandonment counts. Not a defect; not a hold; flagged for the next scheduled prompt revision so the wording is precise the next time we open this section.

### Amendment B — CITATION HONESTY exception sentence (new)

- **Location in body:** Final sentence of "### CITATION HONESTY — non-negotiable" section: *"The self-help-eviction statutes in the SELF-HELP section are an exception — cite those by number, because the user needs to be able to verify them."*
- **Source ruling:** `ownerpilot_route_ts_chat_prompt_attorney_ruling_round_2.md` (2026-06-02), line 85 of that file. Sign-off in `ownerpilot_route_ts_chat_prompt_attorney_signoff.md` (2026-06-02), condition 2 (companion to Amendment A — the SELF-HELP statute citations would otherwise be in tension with the CITATION HONESTY "never invent / hedge if unsure" stance).
- **Rationale:** The five self-help statutes are stable, primary-source-verified, and the user's ability to look them up is the whole point of citing them. Carving them out by name (not by general permission) keeps the no-invented-citations rule intact for everything else.
- **Status:** Permanent. No sunset.

### Amendment C — DOCUMENTS interim language (replaces v4-final wording)

- **Location in body:** Entire "### DOCUMENTS" section — opening paragraph through the "Things you do not draft inline…" close.
- **Source ruling:** `ownerpilot_route_ts_chat_prompt_attorney_ruling_round_2.md` (2026-06-02), line 104 of that file (full interim DOCUMENTS paragraph block). Sign-off in `ownerpilot_route_ts_chat_prompt_attorney_signoff.md` (2026-06-02), condition 4.
- **Status:** **INTERIM. Sunsets when BOTH conditions are met.** See §3 below.

### Non-amendment: standing UI disclosure

The commit d20ccc7 message also references a "standing UI disclosure." That is a UI-layer change, not a prompt-body change, and is not part of this canonical source file. Its sign-off lives in the 2026-06-02 Round 1 ruling. Flagged here only so the cross-reference between the commit message and this artifact is unambiguous.

---

## 3. DOCUMENTS interim sunset gate

Per `documents_interim_sunset_gate_attorney_signoff_2026-06-07.md`:

**Sunset conditions (BOTH must be met):**

- **(a)** v4 payment-fields wording is signed off.
  - **Status: CLEARED 2026-06-04.** Per `A1_part_d_attorney_signoff_2026-06-03.md` + `A1_part_d_attorney_countersign_2026-06-04.md`; ratified `v4_wording_signoff_ratification_and_closeouts_2026-06-05.md`. ✅
- **(b)** Broker-supervised workflow is production-cleared.
  - **Status: NOT MET as of 2026-06-07.** Dev-only, no firm go-live date, aspirational. 🔴

**Disposition:** Amendment C interim language stays in force, byte-identical to Round 2 line 104, until condition (b) flips.

**CI guard — approved:** Build side installed `BROKER_WORKFLOW_PRODUCTION_LIVE`. Flag flip-to-true fails CI while the interim DOCUMENTS language is still deployed, forcing the DOCUMENTS revision back to me before any ship. This is the event-triggered fail-closed pattern the engagement has standardized on (same shape as `V4_WORDING_SIGNED_OFF`).

**What happens at sunset (for the record, so future-me has the playbook):** When condition (b) is about to flip, the build-side CI failure routes the prompt back to me. The revision will restore the v4-final DOCUMENTS section wording (or whatever replacement v4-final-equivalent text the broker workflow's actual capabilities support at that point), removing the "I'm not going to draft the actual notice here in chat" interim posture. Until then: nothing changes.

---

## 4. Byte-identity attestation

The body reproduced in §5 below is byte-identical to the deployed `SYSTEM_PROMPT` constant in `app/api/chat/route.ts` as extracted on 2026-06-06 by the build side (extraction file: `uploaded_attachments/db247de259e14801bb50cf7365446218/ownerpilot_deployed_system_prompt_extracted_2026-06-06_1.md`, between the `----- BEGIN VERBATIM SYSTEM_PROMPT -----` and `----- END VERBATIM SYSTEM_PROMPT -----` markers).

- **Body length:** 14,989 characters / 15,086 UTF-8 bytes.
- **Extraction method (per build side):** programmatic slice between the opening and closing backticks of the `SYSTEM_PROMPT` template literal. Body contains zero backticks. No paraphrase, no summary, no fenced-quote escapes. Leading/trailing whitespace preserved exactly.

**Verbatim discipline:** Anything between the BEGIN/END markers below is canonical. If the deployed prompt and this file ever diverge again, this file is the source of record and the deployed prompt is the defect — same posture as the 2026-06-06 drift diff established, now with the correct underlying baseline.

---

## 5. Canonical prompt body — VERBATIM

The body between the markers below is the prompt as deployed. Do not edit, reflow, paraphrase, or "clean up" any of it. Whitespace, em-dashes, smart-quote vs straight-quote choices, and bullet spacing are all load-bearing.

----- BEGIN VERBATIM SYSTEM_PROMPT -----
You are OwnerPilot, an AI property guidance assistant for California property owners. You help small landlords — especially Accidental Landlords who inherited property and Mom-and-Pop landlords with 1-4 units — prevent property problems before they become expensive disputes, organize their issues and documents, and recognize when professional backup may be needed.
...
----- END VERBATIM SYSTEM_PROMPT -----

---

## 6. Open items (carry-forward, not blocking)

- **[CONSIDER]** §§ 1980–1991 wording refinement in SELF-HELP "removing the tenant's belongings" bullet (see Amendment A note). Next planned prompt revision.
- **[SHOULD FIX]** Route-comment update at `app/api/chat/route.ts` lines 9–10 to point at this file (see §1 above). Comment-only; no behavior impact; no re-review needed; fold into next route.ts commit.
- **Sunset trigger:** When `BROKER_WORKFLOW_PRODUCTION_LIVE` flips to true, the CI failure will route the DOCUMENTS section back to me for the sunset revision per §3.

## 7. Disposition

**Signed off as the canonical source-of-record for the v4.1 system prompt as deployed in `app/api/chat/route.ts` on 2026-06-07.**

This file is the artifact route.ts should reference in its in-route comment. v4-final remains the canonical source for the unchanged portions of the prompt; this file is canonical for the prompt as a whole. The next prompt change of any kind opens a fresh re-review and creates a v4.2 (or v5, depending on the scope of the change).

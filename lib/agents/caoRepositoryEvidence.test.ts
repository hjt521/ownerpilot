...
async function main(): Promise<void> {
  console.log('\nCAO repository evidence collector');
  ...
  await check(
    ...
    async () => {
      ...
      assert.ok(requested.every(url => url.includes(scope.sourceCommit)));
      assert.ok(packet.files.every(file =>

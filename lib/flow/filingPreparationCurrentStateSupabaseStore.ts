...
const DB_ROW_KEYS = [
  'filing_preparation_current_state_id','user_id','riskpath_record_id','revision','state_payload','generated_draft_bytes',
] as const;
...
export interface FilingPreparationCurrentStateSupabaseClient {
  auth: { getUser(): PromiseLike<AuthResponse> };
...
export type ExpectedFilingPreparationCurrentState =
  | { status: 'NONE' }
  | { status: 'CURRENT'; filingPreparationCurrentStateId: string; revision: number };
export type AppendFilingPreparationCurrentStateResult =
  | { status: 'INSERTED'; currentState: FilingPreparationCurrentState }
  | { status: 'CONFLICT'; reloadRequired: true; currentState: null };
...
function requireQueryResponse(value: unknown, operation: string): Record<string, unknown> {
  if (!isPlainObject(value) || !hasOwn(value, 'error')) throw new Error(`Supabase ${operation} returned a malformed response.`);
  return value;
}
...
function requireRiskPathId(value: unknown): string {
  if (typeof value !== 'string' || !UUID_RE.test(value)) throw new Error('RiskPath identity must be an exact UUID.');
  return value;
}
async function requireAuthenticatedUserId(client: FilingPreparationCurrentStateSupabaseClient): Promise<string> {
  ...
  if (!isPlainObject(response.data.user) || typeof response.data.user.id !== 'string' || !UUID_RE.test(response.data.user.id)) {
...
function encodeBytea(value: Uint8Array | null): string | null {
  return value === null ? null : `\\x${Buffer.from(value).toString('hex')}`;
}
...
function validateDurableRow(value: unknown, expectedUserId: string, expectedRiskPathId: string): FilingPreparationCurrentState {
  if (!isPlainObject(value) || !hasExactKeys(value, DB_ROW_KEYS)) throw new Error('Supabase read returned a malformed current-state durable row.');
  ...
  if (typeof stateId !== 'string' || !CURRENT_STATE_ID_RE.test(stateId)) throw new Error('Supabase read returned an invalid current-state identity.');
  if (typeof userId !== 'string' || !UUID_RE.test(userId) || userId !== expectedUserId) throw new Error('Supabase read returned a mismatched authenticated owner identity.');
  if (typeof riskpathRecordId !== 'string' || !UUID_RE.test(riskpathRecordId) || riskpathRecordId !== expectedRiskPathId) throw new Error('Supabase read returned a mismatched RiskPath identity.');
  if (!Number.isSafeInteger(revision) || Number(revision) < 1) throw new Error('Supabase read returned an invalid current-state revision.');
  if (!isPlainObject(statePayload) || hasOwn(statePayload, 'generatedDraftBytes')) throw new Error('Supabase read returned a malformed current-state payload.');
  const reconstructed = { ...statePayload, generatedDraftBytes: decodeBytea(value.generated_draft_bytes) };
  const validated = validateFilingPreparationCurrentState(reconstructed);
  if (validated.status !== 'VALID') throw new Error(`Supabase read returned a noncanonical current-state payload: ${validated.blockReason}.`);
...
async function readLatestForUser(
  ...
  const query = client.from(TABLE).select(READ_COLUMNS)
    .eq('user_id', userId).eq('riskpath_record_id', riskpathRecordId)
...
async function readExactForUser(
  ...
  const query = client.from(TABLE).select(READ_COLUMNS)
    .eq('user_id', userId).eq('riskpath_record_id', riskpathRecordId)
    .eq('revision', revision).eq('filing_preparation_current_state_id', currentStateId).limit(1);
...
function nextRevision(latest: FilingPreparationCurrentState | null): number {
  ...
  if (latest.revision === Number.MAX_SAFE_INTEGER) throw new Error('Current-state revision cannot advance beyond the maximum safe integer.');
...
function expectedCurrentMatches(expectedCurrent: ExpectedFilingPreparationCurrentState, latest: FilingPreparationCurrentState | null): boolean {
...
async function appendFromLatest(
  ...
  const built = createFilingPreparationCurrentState({
    ...
    generatedDraftBinding: input.generatedDraft === null ? null : { revision, generatedDraft: input.generatedDraft },
    generatedDraftBytes: input.generatedDraftBytes,
    currentnessMaterialBinding: input.currentnessMaterialBinding,
    ownerReviewBinding: input.ownerReviewEvidence === null ? null : { revision, ownerReviewEvidence: input.ownerReviewEvidence },
  });
  if (built.status !== 'CURRENT_STATE_REVISION') throw new Error(`Canonical current-state creation blocked: ${built.blockReason}.`);
  const currentState = built.currentState;
  if (currentState.schemaVersion !== 2) throw new Error('Durable append must create only current-state schema v2 revisions.');
  ...
  if (response.error !== null) {
    if (postgresErrorCode(response.error) === '23505') return { status: 'CONFLICT', reloadRequired: true, currentState: null };
    throw new Error('Supabase current-state append failed closed.');
  }
  const readBack = await readExactForUser(client, userId, input.riskpathRecordId, revision, currentState.filingPreparationCurrentStateId);
...
export function createFilingPreparationCurrentStateSupabaseStore(
  ...
  return {
    ...
    async appendNext(input: AppendFilingPreparationCurrentStateInput): Promise<AppendFilingPreparationCurrentStateResult> {
      if (!exactAppendInput(input)) throw new Error('Current-state append input has an invalid shape or contains caller-authored authority fields.');
    ...
    async appendNextIfCurrent(
      ...
      if (!exactExpectedCurrent(expectedCurrent)) throw new Error('Expected current-state identity must be explicit NONE or an exact current-state ID and revision.');
      if (!exactAppendInput(input)) throw new Error('Current-state append input has an invalid shape or contains caller-authored authority fields.');
      const riskpathRecordId = requireRiskPathId(input.riskpathRecordId);
      const userId = await requireAuthenticatedUserId(client);
      const latest = await readLatestForUser(client, userId, riskpathRecordId);
      if (!expectedCurrentMatches(expectedCurrent, latest)) return { status: 'CONFLICT', reloadRequired: true, currentState: null };
      return appendFromLatest(client, userId, latest, input);
    },
  };
}

...
function legacyUuid(v:unknown):string{const b=Buffer.from(createHash('sha256').update(canonicalizeGenerationIdentity(v)).digest().subarray(0,16));b[6]=(b[6]&15)|80;b[8]=(b[8]&63)|128;const h=b.toString('hex');return `${h.slice(0,8)}-${h.slice(8,12)}-${h.slice(12,16)}-${h.slice(16,20)}-${h.slice(20)}`;}
function legacyProfileIds(profileVersion:string,userId:string,generation:string){const b={namespace:SYNTHETIC_QUALIFICATION_NAMESPACE,profileVersion,userId,generation};const binding=createHash('sha256').update(canonicalizeGenerationIdentity(b)).digest('hex');return {riskpathRecordId:legacyUuid({...b,purpose:'riskpath'}),e2eRunId:legacyUuid({...b,purpose:'e2e'}),createdNoticeArtifactId:legacyUuid({...b,purpose:'artifact'}),profileBindingId:`${SYNTHETIC_QUALIFICATION_NAMESPACE}.profile-binding:sha256:${binding}`};}

(async()=>{
  const p=createSyntheticQualificationPreview();
  eq(p.syntheticOnly,true,'preview synthetic only');
  eq(SYNTHETIC_QUALIFICATION_PROFILE_VERSION,'bootstrap-v3','profile version bumped to bootstrap-v3');
  eq(SYNTHETIC_QUALIFICATION_PROFILE_ID,'ownerpilot.synthetic-qualification.e2-3d1.bootstrap-v3','profile id bumped to bootstrap-v3');
  eq(p.profileId,SYNTHETIC_QUALIFICATION_PROFILE_ID,'fixed profile id');
  eq(p.profileVersion,'bootstrap-v3','preview exposes v3 profile');
  ...
  eq(p.fixedElectionProfileReview.leaseStatus,'OTHER','synthetic agreement-applicable classification');
  eq(p.fixedElectionProfileReview.agreementTermDescription,'ONE-YEAR CONTRACT','exact synthetic agreement term');
  eq(p.fixedElectionProfileReview.agreementRentAmount,2500,'exact synthetic agreement rent');
  eq(p.fixedElectionProfileReview.agreementRentFrequency,'MONTHLY','monthly agreement frequency');
  eq(p.fixedElectionProfileReview.agreementRentDue,'FIRST_DAY_OF_MONTH','first-day agreement rent due');
  eq(p.fixedElectionProfileReview.agreementForm,'WRITTEN','written agreement form');
  eq(p.fixedElectionProfileReview.agreementParty,'PLAINTIFF','plaintiff agreement party');
  eq(p.fixedElectionProfileReview.agreementDateStatus,'UNRESOLVED','agreement date remains unresolved');
  ...
  neq(legacyGeneration,p.reviewApprovalGeneration,'v1 and v3 review generations differ');
  const legacyV1Ids=legacyProfileIds('bootstrap-v1',USER,legacyGeneration);
  const legacyV2Ids=legacyProfileIds('bootstrap-v2',USER,p.reviewApprovalGeneration);
  ...
  neq(ids.riskpathRecordId,legacyV1Ids.riskpathRecordId,'v3 RiskPath differs from v1');neq(ids.e2eRunId,legacyV1Ids.e2eRunId,'v3 E2E id differs from v1');neq(ids.createdNoticeArtifactId,legacyV1Ids.createdNoticeArtifactId,'v3 artifact id differs from v1');neq(ids.profileBindingId,legacyV1Ids.profileBindingId,'v3 profile binding differs from v1');
  neq(ids.riskpathRecordId,legacyV2Ids.riskpathRecordId,'bootstrap-v2 RiskPath identity is not reusable');neq(ids.e2eRunId,legacyV2Ids.e2eRunId,'bootstrap-v2 E2E identity is not reusable');neq(ids.createdNoticeArtifactId,legacyV2Ids.createdNoticeArtifactId,'bootstrap-v2 artifact identity is not reusable');neq(ids.profileBindingId,legacyV2Ids.profileBindingId,'bootstrap-v2 profile binding is not reusable');
  ...
  eq(m.syntheticOnly,true,'material synthetic only');eq(m.profileVersion,'bootstrap-v3','material v3 profile');eq(m.riskpathRecordId,ids.riskpathRecordId,'server-derived RiskPath');eq(m.e2eRunId,ids.e2eRunId,'server-derived E2E');eq(m.createdNoticeArtifactId,ids.createdNoticeArtifactId,'server-derived artifact');
  ...
  eq(m.currentnessMaterialBinding.schemaVersion,1,'currentness schema');eq(m.currentnessMaterialBinding.officialSourceHealth,'CURRENT','source CURRENT');eq(m.currentnessMaterialBinding.facts.status,'READY','canonical D.1 READY');eq(fact(m,CANONICAL_FILING_FACT_REFS.propertyStreetAddress).value,'613 E Broadway','canonical premises address');eq(fact(m,CANONICAL_FILING_FACT_REFS.propertyCity).value,'Glendale','canonical premises city');eq(fact(m,CANONICAL_FILING_FACT_REFS.propertyCounty).value,'Los Angeles','canonical premises county');eq(fact(m,CANONICAL_FILING_FACT_REFS.propertyZip).value,'91206','canonical premises ZIP');eq(fact(m,CANONICAL_FILING_FACT_REFS.propertyUnitRepresentation).value.kind,'NO_UNIT','canonical no-unit representation');eq(fact(m,CANONICAL_FILING_FACT_REFS.filerContact).value.streetAddress,'100 Synthetic Qualification Avenue','canonical filer contact remains synthetic');neq(fact(m,CANONICAL_FILING_FACT_REFS.filerContact).value.streetAddress,fact(m,CANONICAL_FILING_FACT_REFS.propertyStreetAddress).value,'canonical filer contact distinct from premises');
  eq(fact(m,CANONICAL_FILING_FACT_REFS.leaseStatus).value,'OTHER','canonical lease status is agreement-applicable OTHER');eq(fact(m,CANONICAL_FILING_FACT_REFS.agreementTermDescription).value,'ONE-YEAR CONTRACT','canonical agreement term preserved');eq(fact(m,CANONICAL_FILING_FACT_REFS.agreementRentAmount).value,2500,'canonical agreement rent preserved separately');eq(fact(m,CANONICAL_FILING_FACT_REFS.agreementRentFrequency).value,'MONTHLY','canonical monthly agreement frequency preserved');eq(fact(m,CANONICAL_FILING_FACT_REFS.agreementRentDue).value,'FIRST_DAY_OF_MONTH','canonical first-day due state preserved');eq(fact(m,CANONICAL_FILING_FACT_REFS.agreementForm).value,'WRITTEN','canonical written agreement preserved');eq(fact(m,CANONICAL_FILING_FACT_REFS.agreementParty).value,'PLAINTIFF','canonical plaintiff agreement party preserved');eq(fact(m,CANONICAL_FILING_FACT_REFS.agreementDate).state,'UNKNOWN','agreement date remains explicit UNKNOWN');eq(fact(m,CANONICAL_FILING_FACT_REFS.leaseApplicabilityControl).value,'AGREEMENT_FIELDS_APPLICABLE','versioned lease control authorizes Item 6 agreement fields');
  for(const ref of [CANONICAL_FILING_FACT_REFS.leaseStatus,CANONICAL_FILING_FACT_REFS.agreementTermDescription,CANONICAL_FILING_FACT_REFS.agreementRentAmount,CANONICAL_FILING_FACT_REFS.agreementRentFrequency,CANONICAL_FILING_FACT_REFS.agreementRentDue,CANONICAL_FILING_FACT_REFS.agreementForm,CANONICAL_FILING_FACT_REFS.agreementParty]){const v=fact(m,ref);ok(!!v.provenance.customerVerification,`${ref} carries explicit customer verification provenance`);ok(String(v.provenance.customerVerification.verificationId).includes('.verification.'),`${ref} verification identity is synthetic-profile bound`);}
  neq(fact(m,CANONICAL_FILING_FACT_REFS.agreementRentAmount).provenance.sourcePaths[0],fact(m,CANONICAL_FILING_FACT_REFS.rentDemandTotal).provenance.sourcePaths[0],'agreement rent authority is semantically distinct from Notice demand authority');
  eq(m.generatedDraft.artifactClass,'GENERATED_DRAFT','canonical E.1 artifact');eq(m.generatedDraft.artifactRole,'OWNER_GENERATED_PREPARATION','canonical artifact role');eq(m.generatedDraft.generatedByteLength,m.generatedDraftBytes.byteLength,'draft evidence binds bytes');eq(m.reviewStatement.statementId,'owner-exact-rendered-document-review-v1','governed owner review statement');

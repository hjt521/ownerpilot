...
export const UD100_GENERATION_BINDING_MAP_VERSION = '1.6.0' as const;
export const UD100_GENERATOR_CONTRACT_VERSION = 'ud100-field-write-plan-v6' as const;
...
const D = (ref: GenerationFactDependency['ref'], authorityClass: GenerationInputAuthorityClass): GenerationFactDependency => ({ ref, authorityClass });
const enumArgs = (allowed: readonly string[], selected: readonly string[]) => ({ allowedValues: allowed.join('|'), selectedValues: selected.join('|') });
function textRule(ev: GenerationFieldEvidence, dep: GenerationFactDependency, transform: GenerationFieldRule extends infer _T ? any : never = { id: 'TEXT_EXACT_V1', version: '1' }, ruleCondition?: GenerationRuleCondition): GenerationFieldRule {
  return { disposition: 'WRITE', evidence: ev, writeKind: 'TEXT', dependencies: [dep], transform, unresolvedPolicy: 'BLOCK', ...(ruleCondition ? { condition: ruleCondition } : {}) };
}
...
function governedNoWrite(ev: GenerationFieldEvidence, dep: GenerationFactDependency, allowedValues: readonly unknown[], reason: string, property?: string): GenerationFieldRule {
  return { disposition: 'GOVERNED_PRESERVE_OFFICIAL_BLANK_NO_WRITE', evidence: ev, dependency: dep, allowedValues, reason, ...(property ? { property } : {}) };
}
function deferred(ev: GenerationFieldEvidence, reason: string): GenerationFieldRule {
  return { disposition: 'PRESERVE_OFFICIAL_BLANK_NO_WRITE', evidence: ev, authorityClass: 'DEFERRED_TO_LATER_STAGE_NOT_WRITABLE_BY_D1', reason };
}
function nondata(ev: GenerationFieldEvidence): GenerationFieldRule {
  return { disposition: 'PRESERVE_OFFICIAL_BLANK_NO_WRITE', evidence: ev, authorityClass: 'OFFICIAL_FORM_NONDATA_CONTROL_NOT_WRITABLE', reason: 'Official PDF action control is not a customer preparation field and is never mutated by the whitelist generator.' };
}
function condition(dep: GenerationFactDependency, allowedValues: readonly unknown[], reason: string, property?: string): GenerationRuleCondition {
  return { dependency: dep, allowedValues, whenFalse: 'PRESERVE_OFFICIAL_BLANK_NO_WRITE', reason, ...(property ? { property } : {}) };
}
...
const noticeDomain = ['PAY_RENT_OR_QUIT_3_DAY','QUIT_30_DAY','QUIT_60_DAY','QUIT_3_DAY','CARES_30_DAY','PERFORM_COVENANTS_3_DAY','PRIOR_1946_2_C','OTHER'] as const;
const serviceDomain = ['PERSONAL_HAND_DELIVERY','SUBSTITUTED_SERVICE','POST_AND_MAIL','CERTIFIED_OR_REGISTERED_MAIL','COMMERCIAL_LEASE_METHOD'] as const;
const leaseDomain = ['NO_AGREEMENT','MONTH_TO_MONTH','FIXED_TERM','OTHER'] as const;
const leaseApplicabilityDomain = ['NO_AGREEMENT_FIELDS_NOT_APPLICABLE','AGREEMENT_FIELDS_APPLICABLE'] as const;
const agreementFormDomain = ['WRITTEN','ORAL'] as const;
const agreementPartyDomain = ['PLAINTIFF','PLAINTIFF_AGENT','PLAINTIFF_PREDECESSOR','OTHER'] as const;
const agreementFrequencyDomain = ['MONTHLY','OTHER'] as const;
const agreementDueDomain = ['FIRST_DAY_OF_MONTH','OTHER_DAY'] as const;
...
const leaseDep = D(CANONICAL_FILING_FACT_REFS.leaseStatus, CUSTOMER);
const leaseApplicabilityDep = D(CANONICAL_FILING_FACT_REFS.leaseApplicabilityControl, CONTROL);
const agreementCondition = condition(leaseApplicabilityDep, ['AGREEMENT_FIELDS_APPLICABLE'], 'Verified NO_AGREEMENT makes Item 6 agreement-only fields not applicable.');
...
const domain1Relationship: GenerationFieldRule[] = [
  ...
  governedNoWrite(evidence('UD-100[0].Page1[0].List4[0].FillText120[0]',1,'/Tx','786 0 R','(specify):'),D(CANONICAL_FILING_FACT_REFS.plaintiffRelationship,CUSTOMER),['OWNER'],'Current bounded profile supports owner relationship; other-relationship detail cannot be silently omitted.'),
];
const domain1Dba: GenerationFieldRule[] = [
  governedNoWrite(evidence('UD-100[0].Page1[0].List2[0].Item2[0].Lib[0].FillText4[0]',1,'/Tx','803 0 R','(specify):'),D(CANONICAL_FILING_FACT_REFS.dbaUse,CUSTOMER),['NO_DBA'],'Explicit customer-confirmed NO_DBA authorizes no DBA detail write.'),
  checkboxEnumRule(evidence('UD-100[0].Page1[0].List2[0].Item2[0].Lib[0].CBChoice1_cb[0]',1,'/Btn','804 0 R','Plaintiff has complied with the fictitious business name laws and is doing business under the fictitious name of'),D(CANONICAL_FILING_FACT_REFS.dbaUse,CUSTOMER),['NO_DBA','USES_DBA'],['USES_DBA']),
];
...
const defendantEvidence = [
  evidence('UD-100[0].Page1[0].P1Caption[0].TitlePartyName[0].Party2_ft[0]',1,'/Tx','837 0 R','DEFENDANT:'),evidence('UD-100[0].Page1[0].List1[0].FillText2[0]',1,'/Tx','818 0 R','alleges causes of action against DEFENDANT (name each):'),evidence('UD-100[0].Page2[0].Header[0].TitlePartyName[0].Party2_ft[0]',2,'/Tx','777 0 R','DEFENDANT:'),evidence('UD-100[0].Page3[0].Header[0].TitlePartyName[0].Party2_ft[0]',3,'/Tx','669 0 R','DEFENDANT:'),evidence('UD-100[0].Page4[0].Header[0].TitlePartyName[0].Party2_ft[0]',4,'/Tx','911 0 R','DEFENDANT:'),
] as const;
const domain1Parties: GenerationFieldRule[] = [...partyEvidence.map(item=>textRule(item,D(CANONICAL_FILING_FACT_REFS.plaintiffNames,CUSTOMER),{id:'TEXT_ARRAY_SEMICOLON_V1',version:'1'})),...defendantEvidence.map(item=>textRule(item,D(CANONICAL_FILING_FACT_REFS.defendantNames,CUSTOMER),{id:'TEXT_ARRAY_SEMICOLON_V1',version:'1'}))];
const domain1Does: GenerationFieldRule[] = [
  objectBooleanRule(evidence('UD-100[0].Page1[0].P1Caption[0].TitlePartyName[0].Does[0]',1,'/Btn','838 0 R','DOES 1'),D(CANONICAL_FILING_FACT_REFS.doeElection,ELECTION),'include',true),
  governedNoWrite(evidence('UD-100[0].Page1[0].P1Caption[0].TitlePartyName[0].FillText140[0]',1,'/Tx','839 0 R','TO'),D(CANONICAL_FILING_FACT_REFS.doeElection,ELECTION),[false],'Explicit customer-confirmed no-Doe election authorizes no Doe range write.','include'),
];
const courtRules: GenerationFieldRule[] = [
  ['CrtCounty_ft[0]','840 0 R','SUPERIOR COURT OF CALIFORNIA, COUNTY OF','county'],['Street_ft[0]','841 0 R','STREET ADDRESS:','streetAddress'],['MailingAdd_ft[0]','842 0 R','MAILING ADDRESS:','mailingAddress'],['CityZip_ft[0]','843 0 R','CITY AND ZIP CODE:','cityAndZip'],['Branch_ft[0]','844 0 R','BRANCH NAME:','branchName'],
].map(([leaf,objectReference,label,property])=>textRule(evidence(`UD-100[0].Page1[0].P1Caption[0].CourtInfo[0].${leaf}`,1,'/Tx',objectReference,label),D(CANONICAL_FILING_FACT_REFS.selectedFilingCourt,ELECTION),{id:'OBJECT_PROPERTY_TEXT_V1',version:'1',args:{property}}));
...
const domain2Premises: GenerationFieldRule[] = [
  ...
  textRule(evidence('UD-100[0].Page1[0].List3[0].SubList3[0].Lib[0].SubListb[0].Lii2[0].FillText12[0]',1,'/Tx','796 0 R','(name of county):'),D(CANONICAL_FILING_FACT_REFS.propertyCounty,CUSTOMER),{id:'TEXT_EXACT_V1',version:'1'},condition(municipalDep,['UNINCORPORATED_AREA'],'Resolved city-limits classification makes the unincorporated-county detail not applicable.')),
  ...
  textRule(evidence('UD-100[0].Page1[0].List3[0].SubList3[0].Lib[0].SubListb[0].Lii1[0].FillText10[0]',1,'/Tx','798 0 R','(name of city):'),D(CANONICAL_FILING_FACT_REFS.propertyCity,CUSTOMER),{id:'TEXT_EXACT_V1',version:'1'},condition(municipalDep,['WITHIN_CITY_LIMITS'],'Resolved unincorporated classification makes the city-limits detail not applicable.')),
  { disposition:'WRITE', evidence:evidence('UD-100[0].Page1[0].List3[0].SubList3[0].Lia[0].FillText6[0]',1,'/Tx','799 0 R','The venue is the court named above because defendant named above is in possession of the premises located at (street address, apartment number, city, zip code, and county):'), writeKind:'TEXT', dependencies:[D(CANONICAL_FILING_FACT_REFS.propertyStreetAddress,CUSTOMER),D(CANONICAL_FILING_FACT_REFS.propertyUnitRepresentation,CUSTOMER),D(CANONICAL_FILING_FACT_REFS.propertyCity,CUSTOMER),D(CANONICAL_FILING_FACT_REFS.propertyZip,CUSTOMER),D(CANONICAL_FILING_FACT_REFS.propertyCounty,CUSTOMER)], transform:{id:'PREMISES_COMPOSE_V2',version:'2'}, unresolvedPolicy:'BLOCK' },
  ...courtRules,
];
...
const domain3Civil: GenerationFieldRule[] = [
  ...
  ...[evidence('UD-100[0].Page1[0].CheckAll[0].ActionIs[0]',1,'/Btn','823 0 R','ACTION IS RECLASSIFIED by this amended complaint or cross-complaint'),evidence('UD-100[0].Page1[0].CheckAll[0].Fromunlawful[0]',1,'/Btn','824 0 R','from unlawful detainer to general unlimited civil'),evidence('UD-100[0].Page1[0].CheckAll[0].Fromunlawful[1]',1,'/Btn','825 0 R','from unlawful detainer to general limited civil'),evidence('UD-100[0].Page1[0].CheckAll[0].FromUnltd[0]',1,'/Btn','826 0 R','from limited to unlimited.'),evidence('UD-100[0].Page1[0].CheckAll[0].FromUnltd[1]',1,'/Btn','827 0 R','from unlimited to limited.')].map(ev=>governedNoWrite(ev,lifecycleDep,['INITIAL_PREFILING'],'Exact lifecycle proves this is an initial pre-filing complaint; reclassification fields are not applicable.')),
  ...
  ...[evidence('UD-100[0].Page1[0].P1Caption[0].CaseNumber[0].CaseNumber_ft[0]',1,'/Tx','856 0 R','CASE NUMBER:'),evidence('UD-100[0].Page2[0].Header[0].CaseNumber[0].CaseNumber_ft[0]',2,'/Tx','775 0 R','CASE NUMBER:'),evidence('UD-100[0].Page3[0].Header[0].CaseNumber[0].CaseNumber_ft[0]',3,'/Tx','667 0 R','CASE NUMBER:'),evidence('UD-100[0].Page4[0].Header[0].CaseNumber[0].CaseNumber_ft[0]',4,'/Tx','909 0 R','CASE NUMBER:')].map(ev=>deferred(ev,'Pre-filing case number is expressly deferred; official blank is preserved until a court-issued event exists.')),
];

const domain4Lease: GenerationFieldRule[] = [
  deferred(evidence('UD-100[0].Page2[0].List6[0].SubList6[0].Lif[0].SixF[0]',2,'/Btn','726 0 R','A copy of the written agreement is not attached because'),'Agreement attachment state remains packet-composition dependent.'),
  deferred(evidence('UD-100[0].Page2[0].List6[0].SubList6[0].Lif[0].SubListf[0].Li2[0].SixF124[0]',2,'/Btn','730 0 R','this action is solely for nonpayment of rent'),'Agreement attachment exception is not inferred from notice type.'),
  deferred(evidence('UD-100[0].Page2[0].List6[0].SubList6[0].Lif[0].SubListf[0].Li1[0].SixF123[0]',2,'/Btn','731 0 R','the written agreement is not in the possession of the landlord'),'Agreement possession is unresolved and is not fabricated.'),
  deferred(evidence('UD-100[0].Page2[0].List6[0].SubList6[0].Lid[0].SixD[0]',2,'/Btn','734 0 R','The agreement was later changed as follows'),'Agreement-change state is not supplied by the bounded synthetic facts.'),
  deferred(evidence('UD-100[0].Page2[0].List6[0].SubList6[0].Lid[0].fl1001\\.29[0]',2,'/Tx','735 0 R','(specify):'),'Agreement-change detail is not supplied by the bounded synthetic facts.'),
  deferred(evidence('UD-100[0].Page2[0].List6[0].SubList6[0].Lic[0].SixC[0]',2,'/Btn','736 0 R','The defendants not named in item 6a are'),'All governed defendant identities are written to Item 6a; no additional defendant classification is inferred.'),
  deferred(evidence('UD-100[0].Page2[0].List6[0].SubList6[0].Lic[0].SubListc[0].Li3[0].SixC13[0]',2,'/Btn','741 0 R','Other'),'No unlisted-defendant category is inferred.'),
  deferred(evidence('UD-100[0].Page2[0].List6[0].SubList6[0].Lic[0].SubListc[0].Li3[0].FillText113[0]',2,'/Tx','742 0 R','(specify):'),'No unlisted-defendant detail is inferred.'),
  deferred(evidence('UD-100[0].Page2[0].List6[0].SubList6[0].Lic[0].SubListc[0].Li2[0].SixC13[0]',2,'/Btn','743 0 R','assignees.'),'No assignee status is inferred.'),
  deferred(evidence('UD-100[0].Page2[0].List6[0].SubList6[0].Lic[0].SubListc[0].Li1[0].SixC13[0]',2,'/Btn','744 0 R','subtenants.'),'No subtenant status is inferred.'),
  checkboxEnumRule(evidence('UD-100[0].Page2[0].List6[0].SubList6[0].Lib[0].SixB[0]',2,'/Btn','745 0 R','written'),D(CANONICAL_FILING_FACT_REFS.agreementForm,CUSTOMER),agreementFormDomain,['WRITTEN'],agreementCondition),
  checkboxEnumRule(evidence('UD-100[0].Page2[0].List6[0].SubList6[0].Lib[0].SixB[1]',2,'/Btn','746 0 R','oral'),D(CANONICAL_FILING_FACT_REFS.agreementForm,CUSTOMER),agreementFormDomain,['ORAL'],agreementCondition),
  checkboxEnumRule(evidence('UD-100[0].Page2[0].List6[0].SubList6[0].Lib[0].SubListb[0].Li4[0].SixB14[0]',2,'/Btn','752 0 R','Other'),D(CANONICAL_FILING_FACT_REFS.agreementParty,CUSTOMER),agreementPartyDomain,['OTHER'],agreementCondition),
  deferred(evidence('UD-100[0].Page2[0].List6[0].SubList6[0].Lib[0].SubListb[0].Li4[0].FillText113[0]',2,'/Tx','753 0 R','(specify):'),'Other-party detail remains blank unless a separately released generation branch supports it.'),
  checkboxEnumRule(evidence('UD-100[0].Page2[0].List6[0].SubList6[0].Lib[0].SubListb[0].Li3[0].SixB14[0]',2,'/Btn','754 0 R',"plaintiff's predecessor in interest."),D(CANONICAL_FILING_FACT_REFS.agreementParty,CUSTOMER),agreementPartyDomain,['PLAINTIFF_PREDECESSOR'],agreementCondition),
  checkboxEnumRule(evidence('UD-100[0].Page2[0].List6[0].SubList6[0].Lib[0].SubListb[0].Li2[0].SixB14[0]',2,'/Btn','755 0 R',"plaintiff's agent."),D(CANONICAL_FILING_FACT_REFS.agreementParty,CUSTOMER),agreementPartyDomain,['PLAINTIFF_AGENT'],agreementCondition),
  checkboxEnumRule(evidence('UD-100[0].Page2[0].List6[0].SubList6[0].Lib[0].SubListb[0].Li1[0].SixB14[0]',2,'/Btn','756 0 R','plaintiff.'),D(CANONICAL_FILING_FACT_REFS.agreementParty,CUSTOMER),agreementPartyDomain,['PLAINTIFF'],agreementCondition),
  deferred(evidence('UD-100[0].Page2[0].List6[0].SubList6[0].Lia[0].DateField12[0]',2,'/Tx','757 0 R','On or about (date):'),'Agreement date is explicitly unresolved; no date is fabricated or derived.'),
  textRule(evidence('UD-100[0].Page2[0].List6[0].SubList6[0].Lia[0].FillText22[0]',2,'/Tx','758 0 R','defendant (name each):'),D(CANONICAL_FILING_FACT_REFS.defendantNames,CUSTOMER),{id:'TEXT_ARRAY_SEMICOLON_V1',version:'1'},agreementCondition),
  checkboxEnumRule(evidence('UD-100[0].Page2[0].List6[0].SubList6[0].Lia[0].SubLista[0].Li3[0].SixA3[0]',2,'/Btn','763 0 R','first of the month'),D(CANONICAL_FILING_FACT_REFS.agreementRentDue,CUSTOMER),agreementDueDomain,['FIRST_DAY_OF_MONTH'],agreementCondition),
  checkboxEnumRule(evidence('UD-100[0].Page2[0].List6[0].SubList6[0].Lia[0].SubLista[0].Li3[0].SixA3[1]',2,'/Btn','764 0 R','other day'),D(CANONICAL_FILING_FACT_REFS.agreementRentDue,CUSTOMER),agreementDueDomain,['OTHER_DAY'],agreementCondition),
  deferred(evidence('UD-100[0].Page2[0].List6[0].SubList6[0].Lia[0].SubLista[0].Li3[0].FillText114[0]',2,'/Tx','765 0 R','(specify):'),'Other-day detail is outside the bounded synthetic profile and is not fabricated.'),
  textRule(evidence('UD-100[0].Page2[0].List6[0].SubList6[0].Lia[0].SubLista[0].Li2[0].dollar[0]',2,'/Tx','766 0 R','agreed to pay rent of Dollar amount'),D(CANONICAL_FILING_FACT_REFS.agreementRentAmount,CUSTOMER),{id:'NUMBER_TEXT_V1',version:'1'},agreementCondition),
  checkboxEnumRule(evidence('UD-100[0].Page2[0].List6[0].SubList6[0].Lia[0].SubLista[0].Li2[0].SixA2[0]',2,'/Btn','767 0 R','monthly'),D(CANONICAL_FILING_FACT_REFS.agreementRentFrequency,CUSTOMER),agreementFrequencyDomain,['MONTHLY'],agreementCondition),
  checkboxEnumRule(evidence('UD-100[0].Page2[0].List6[0].SubList6[0].Lia[0].SubLista[0].Li2[0].SixA2[1]',2,'/Btn','768 0 R','other'),D(CANONICAL_FILING_FACT_REFS.agreementRentFrequency,CUSTOMER),agreementFrequencyDomain,['OTHER'],agreementCondition),
  deferred(evidence('UD-100[0].Page2[0].List6[0].SubList6[0].Lia[0].SubLista[0].Li2[0].FillText115[0]',2,'/Tx','769 0 R','(specify frequency):'),'Other-frequency detail is outside the bounded synthetic profile and is not fabricated.'),
  checkboxEnumRule(evidence('UD-100[0].Page2[0].List6[0].SubList6[0].Lia[0].SubLista[0].Li1[0].SixA1[0]',2,'/Btn','770 0 R','month-to-month tenancy'),leaseDep,leaseDomain,['MONTH_TO_MONTH'],agreementCondition),
  checkboxEnumRule(evidence('UD-100[0].Page2[0].List6[0].SubList6[0].Lia[0].SubLista[0].Li1[0].SixA1[1]',2,'/Btn','771 0 R','other tenancy'),leaseDep,leaseDomain,['FIXED_TERM','OTHER'],agreementCondition),
  textRule(evidence('UD-100[0].Page2[0].List6[0].SubList6[0].Lia[0].SubLista[0].Li1[0].FillText116[0]',2,'/Tx','772 0 R','(specify):'),D(CANONICAL_FILING_FACT_REFS.agreementTermDescription,CUSTOMER),{id:'TEXT_EXACT_V1',version:'1'},condition(leaseDep,['FIXED_TERM','OTHER'],'Month-to-month or no-agreement classification requires no Item 6a other-tenancy detail.')),
  deferred(evidence('UD-100[0].Page4[0].List16[0].Thirteen[0]',4,'/Btn','906 0 R','A written agreement between the parties provides for attorney fees.'),'No attorney-fee agreement term is supplied; Item 13 remains source-native blank.'),
...
const domain4Notice: GenerationFieldRule[] = [
  ...
  textRule(evidence('UD-100[0].Page3[0].List13[0].FillText209[0]',3,'/Tx','606 0 R','Dollar Amount'),D(CANONICAL_FILING_FACT_REFS.rentDueAtService,CUSTOMER),{id:'NUMBER_TEXT_V1',version:'1'},condition(noticeElectionDep,['PAY_RENT_OR_QUIT_3_DAY'],'Rent-due-at-service amount is not applicable to a different confirmed complaint notice election.')),
...
const domain4Service: GenerationFieldRule[] = [
  ...
  governedNoWrite(evidence('UD-100[0].Page3[0].List10[0].Item10[0].LI2[0].Eightb[0]',3,'/Btn','629 0 R','(Name):'),serviceElectionDep,['PERSONAL_HAND_DELIVERY'],'Joint-written-agreement service branch is not selected by the confirmed personal-delivery election.'),
  governedNoWrite(evidence('UD-100[0].Page3[0].List10[0].Item10[0].LI2[0].FillText158[0]',3,'/Tx','630 0 R','enter name'),serviceElectionDep,['PERSONAL_HAND_DELIVERY'],'Joint-written-agreement service-name detail is not selected by the confirmed personal-delivery election.'),
  ...
  textRule(evidence('UD-100[0].Page3[0].List10[0].Item10[0].Lia[0].SubLista[0].Li1[0].DateField21[0]',3,'/Tx','653 0 R','on (date):'),serviceFactsDep,{id:'OBJECT_PROPERTY_TEXT_V1',version:'1',args:{property:'serviceDate'}},condition(serviceElectionDep,['PERSONAL_HAND_DELIVERY'],'Personal-delivery date is not applicable to another complaint service election.')),
];
...
const domain5Relief: GenerationFieldRule[] = [
  ...
  textRule(evidence('UD-100[0].Page4[0].List20[0].Item20[0].Lic[0].FillText36[0]',4,'/Tx','902 0 R','Dollar Amount'),D(CANONICAL_FILING_FACT_REFS.pastDueRentRelief,ELECTION),{id:'OBJECT_PROPERTY_NUMBER_TEXT_V1',version:'1',args:{property:'amount'}},condition(D(CANONICAL_FILING_FACT_REFS.pastDueRentRelief,ELECTION),[true],'Explicit owner nonselection of past-due-rent relief authorizes no amount write.','selected')),
...
const domain6Uda: GenerationFieldRule[] = [
  ...
  ...[evidence('UD-100[0].Page4[0].List22[0].Item22[0].Lif[0].DateField26[0]',4,'/Tx','876 0 R','Expires on (date):'),evidence('UD-100[0].Page4[0].List22[0].Item22[0].Lie[0].TextField43[0]',4,'/Tx','877 0 R','Registration Number:'),evidence('UD-100[0].Page4[0].List22[0].Item22[0].Lid[0].TextField42[0]',4,'/Tx','878 0 R','County of registration:'),evidence('UD-100[0].Page4[0].List22[0].Item22[0].Lic[0].Phone_ft[0]',4,'/Tx','879 0 R','Telephone Number:'),evidence('UD-100[0].Page4[0].List22[0].Item22[0].Lib[0].TextField41[0]',4,'/Tx','880 0 R','Street address, city, and zip code:'),evidence('UD-100[0].Page4[0].List22[0].Item22[0].Lia[0].TextField40[0]',4,'/Tx','881 0 R',"Assistant's name:")].map(ev=>governedNoWrite(ev,udaDep,['NO_COMPENSATED_ASSISTANT'],'Current validated no-compensated-assistant control expressly authorizes no paid UDA/LDA credential write.')),
];
const nondataRules: GenerationFieldRule[] = [nondata(evidence('UD-100[0].#pageSet[0].MPLast[0].#area[0].Print[0]',4,'/Btn','590 0 R','Print')),nondata(evidence('UD-100[0].#pageSet[0].MPLast[0].#area[0].Save[0]',4,'/Btn','591 0 R','Save')),nondata(evidence('UD-100[0].#pageSet[0].MPLast[0].#area[0].Reset[0]',4,'/Btn','592 0 R','Clear'))];
...
const semantics:OfficialFormGenerationBindingSemantics={generationSchemaVersion:2,mapId:UD100_GENERATION_BINDING_MAP_ID,mapVersion:UD100_GENERATION_BINDING_MAP_VERSION,profileId:UD100_GENERATION_PROFILE_ID,generatorContractVersion:UD100_GENERATOR_CONTRACT_VERSION,sourceIdentity:UD100_OFFICIAL_SOURCE_IDENTITY,artifactRole:'OWNER_GENERATED_PREPARATION',fieldRules,profileRequirements:[
  ...
  {dependency:leaseDep,allowedValues:leaseDomain,blockerCode:'LEASE_STATUS_CUSTOMER_FACT_REQUIRED'},
  {dependency:leaseApplicabilityDep,allowedValues:leaseApplicabilityDomain,blockerCode:'LEASE_AGREEMENT_FIELDS_REQUIRE_EXACT_CUSTOMER_FACT_BINDING',requiredProvenanceDependencies:[CANONICAL_FILING_FACT_REFS.leaseStatus]},
...
export const UD100_PROHIBITED_SEMANTIC_SUBSTITUTIONS=Object.freeze([
  ...
  {semanticTarget:'COMPLAINT_DEMAND_OR_CIVIL_CLASSIFICATION',prohibitedSourceRef:CANONICAL_FILING_FACT_REFS.rentDemandTotal,reason:'Notice demand is not complaint demand or limited/unlimited classification input by label similarity.'},
  {semanticTarget:'FAIR_RENTAL_VALUE_OR_DAMAGES',prohibitedSourceRef:CANONICAL_FILING_FACT_REFS.rentDemandTotal,reason:'Notice demand is not fair rental value or damages.'},

...
import { evaluateUd100GeneratedDraftLegacyB1Currentness, generateUd100GeneratedDraftLegacyB1, UD100_PREPARATION_RUNTIME_PATH } from './ud100GeneratedDraft';
...
const timestamptzRowKeys = new Set<(typeof rowKeys)[number]>(['transcript_snapshot_at','created_notice_finalized_at']);
...
function timestampInstant(v: unknown): string|null { if(typeof v!=='string')return null; const m=ISO_TIMESTAMP_WITH_ZONE.exec(v); if(!m)return null; const year=Number(m[1]),month=Number(m[2]),day=Number(m[3]),hour=Number(m[4]),minute=Number(m[5]),second=Number(m[6]),fraction=(m[7]??'').padEnd(9,'0'),offsetHour=Number(m[9]??0),offsetMinute=Number(m[10]??0); if(month<1||month>12||day<1||hour>23||minute>59||second>59||offsetHour>14||offsetMinute>59||(offsetHour===14&&offsetMinute!==0))return null; const leap=year%4===0&&(year%100!==0||year%400===0),maxDay=[31,leap?29:28,31,30,31,30,31,31,30,31,30,31][month-1]; if(day>maxDay)return null; const adjustedYear=year-(month<=2?1:0),era=Math.floor(adjustedYear/400),yoe=adjustedYear-era*400,shiftedMonth=month+(month>2?-3:9),doy=Math.floor((153*shiftedMonth+2)/5)+day-1,days=era*146097+yoe*365+Math.floor(yoe/4)-Math.floor(yoe/100)+doy-719468,offsetSeconds=(m[8]==='-'?-1:1)*(offsetHour*3600+offsetMinute*60),utcSeconds=days*86400+hour*3600+minute*60+second-offsetSeconds; return `${utcSeconds}:${fraction}`; }
...
export async function materializeSyntheticQualification(input:{authenticatedUserId:string;reviewApprovalGeneration:string;ceremonyAtISO:string}):Promise<any>{
  ...
  const gen=await generateUd100GeneratedDraftLegacyB1({officialSourceIdentity:UD100_OFFICIAL_SOURCE_IDENTITY,officialSourceHealth:'CURRENT',officialSourceBytes:source,preparationAuthorization:auth,preparationDerivativeBytes:derivative,facts,preparedAtISO:input.ceremonyAtISO}); if(gen.status!=='GENERATED_DRAFT') throw new Error(`Synthetic qualification generated draft blocked: ${gen.blockReason}.`);
  const cur=evaluateUd100GeneratedDraftLegacyB1Currentness(gen.evidence,{officialSourceIdentity:UD100_OFFICIAL_SOURCE_IDENTITY,officialSourceHealth:'CURRENT',officialSourceBytes:source,preparationAuthorization:auth,preparationDerivativeBytes:derivative,facts,draftBytes:gen.bytes}); if(cur.status!=='CURRENT') throw new Error(`Synthetic qualification generated draft is not CURRENT: ${cur.reasons.join('; ')}`);
...
export function exactSyntheticBootstrapRowMatch(actual:unknown,expected:any):boolean{ if(!plain(actual))return false; return rowKeys.every(k=>{ if(!(k in actual))return false; if(timestamptzRowKeys.has(k)){const a=timestampInstant(actual[k]),e=timestampInstant(expected[k]);return a!==null&&e!==null&&a===e;} return canonicalizeGenerationIdentity(actual[k])===canonicalizeGenerationIdentity(expected[k]); }); }

...
import { evaluateUd100BootstrapV3CompatibleDraftCurrentness, generateUd100BootstrapV3CompatibleDraft, UD100_PREPARATION_RUNTIME_PATH } from './ud100GeneratedDraft';
...
export async function materializeSyntheticQualification(input:{authenticatedUserId:string;reviewApprovalGeneration:string;ceremonyAtISO:string}):Promise<any>{
  ...
  const gen=await generateUd100BootstrapV3CompatibleDraft({officialSourceIdentity:UD100_OFFICIAL_SOURCE_IDENTITY,officialSourceHealth:'CURRENT',officialSourceBytes:source,preparationAuthorization:auth,preparationDerivativeBytes:derivative,facts,preparedAtISO:input.ceremonyAtISO}); if(gen.status!=='GENERATED_DRAFT') throw new Error(`Synthetic qualification generated draft blocked: ${gen.blockReason}.`);
  const cur=evaluateUd100BootstrapV3CompatibleDraftCurrentness(gen.evidence,{officialSourceIdentity:UD100_OFFICIAL_SOURCE_IDENTITY,officialSourceHealth:'CURRENT',officialSourceBytes:source,preparationAuthorization:auth,preparationDerivativeBytes:derivative,facts,draftBytes:gen.bytes}); if(cur.status!=='CURRENT') throw new Error(`Synthetic qualification generated draft is not CURRENT: ${cur.reasons.join('; ')}`);

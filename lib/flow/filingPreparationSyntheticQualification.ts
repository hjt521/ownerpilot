...
function timestampInstant(v: unknown): number|null { if(typeof v!=='string')return null; const m=ISO_TIMESTAMP_WITH_ZONE.exec(v); if(!m)return null; const year=Number(m[1]),month=Number(m[2]),day=Number(m[3]),hour=Number(m[4]),minute=Number(m[5]),second=Number(m[6]),offsetHour=Number(m[8]??0),offsetMinute=Number(m[9]??0); if(month<1||month>12||day<1||hour>23||minute>59||second>59||offsetHour>14||offsetMinute>59||(offsetHour===14&&offsetMinute!==0))return null; const maxDay=new Date(Date.UTC(year,month,0)).getUTCDate(); if(day>maxDay)return null; const t=Date.parse(v); return Number.isFinite(t)?t:null; }

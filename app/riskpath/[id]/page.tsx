...
export default async function ExactRiskPathRecordPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Exact route id + exact current user + live-row filter. Service-role access never bypasses this check.
  const { data: record } = await sb
    .from('riskpath_records')
    .select('id, user_id, current_state, notice_document_id, counsel_route_trigger, created_at, updated_at, chat_session_id, captured_payload, produce_snapshot, produce_audit')
    .eq('id', id)
  ...
  if (snapshot && record.chat_session_id) {
    const { data: sourceSession } = await sb
      .from('chat_sessions')
      .select('intake_state')
      .eq('id', record.chat_session_id)
      .maybeSingle();
    ...
    if (intake) {
      try {
        ...
        // Preserve the existing dashboard posture: inability to reassemble does not invent a stale verdict.
      }
    }
  }
  ...
  const task = resolveOwnerContinuationTask({
    currentState: record.current_state,
    stale,
    lahdEligible,
    lahdFiled,
  });
  const stateLabel = record.current_state === 'notice_created'
    ? NOTICE_CREATED_DISPLAY_STATE
    : statusMeta(record.current_state).label;
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <main className="mx-auto w-full max-w-2xl px-5 py-10">
        ...
        {(property || tenants) && (
          <div className="mt-5 rounded-lg border border-rule bg-white p-4">
        ...
        <section className="mt-5 rounded-lg border border-rule bg-white p-5">
          ...
          {task.kind === 'existing_staleness_action' && (
            <>
              {staleWarning && <p className="mt-2 text-sm leading-relaxed text-amber-800">{staleWarning}</p>}
              <a href="/chat/review" className="mt-4 inline-flex min-h-[48px] items-center rounded-md bg-ink px-5 py-3 text-sm font-semibold text-white">
                Review &amp; produce a new notice
              </a>
            </>
          )}
          {task.kind === 'existing_lahd_action' && (
            <a href="/riskpath" className="mt-4 inline-flex min-h-[48px] items-center rounded-md bg-ink px-5 py-3 text-sm font-semibold text-white">
              Open LAHD filing record
            </a>
          )}
          {task.kind === 'review_record' && (
            <p className="mt-2 text-sm leading-relaxed text-gray-700">Review the stored record information below. No new legal or operational step is selected here.</p>
          )}
        </section>
        ...
        <section className="mt-5 rounded-lg border border-rule bg-white p-5">
          ...
          <dl className="mt-3 space-y-2 text-sm">
            <div className="flex justify-between gap-4"><dt className="text-gray-500">Created</dt><dd>{new Date(record.created_at).toLocaleDateString()}</dd></div>
            <div className="flex justify-between gap-4"><dt className="text-gray-500">Last updated</dt><dd>{new Date(record.updated_at).toLocaleDateString()}</dd></div>
          </dl>
          {record.notice_document_id && (
            <a href={`/api/documents/${record.notice_document_id}`} className="mt-4 inline-block min-h-[44px] text-sm font-medium underline">
              View notice PDF
            </a>
          )}
        </section>
